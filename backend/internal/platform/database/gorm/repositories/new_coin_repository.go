...
func (r *GORMNewCoinRepository) Delete(ctx context.Context, symbol string) error {
	...
	if result.Error != nil {
		r.logger.Error("Failed to delete new coin", zap.String("symbol", symbol), zap.Error(result.Error))
		return result.Error
	}
	return nil
}

// FindByID returns a new coin by ID
...
// FindByDateRange returns new coins found within a date range
func (r *GORMNewCoinRepository) FindByDateRange(ctx context.Context, startDate, endDate time.Time) ([]*models.NewCoin, error) {
	var coins []*models.NewCoin
	result := r.db.WithContext(ctx).
		Where("found_at BETWEEN ? AND ? AND is_deleted = ?", startDate, endDate, false).
		Find(&coins)
	if result.Error != nil {
...
// FindByDate returns new coins found on a specific date
...
// FindUpcoming returns upcoming new coins
func (r *GORMNewCoinRepository) FindUpcoming(ctx context.Context) ([]*models.NewCoin, error) {
	var coins []*models.NewCoin
	result := r.db.WithContext(ctx).
		Where("is_upcoming = ? AND is_deleted = ?", true, false).
		Find(&coins)
	if result.Error != nil {
...
// FindUpcomingByDate returns upcoming new coins for a specific date
func (r *GORMNewCoinRepository) FindUpcomingByDate(ctx context.Context, date time.Time) ([]*models.NewCoin, error) {
	...
	var coins []*models.NewCoin
	result := r.db.WithContext(ctx).
		Where("is_upcoming = ? AND first_open_time BETWEEN ? AND ? AND is_deleted = ?", 
			true, startOfDay, endOfDay, false).
		Find(&coins)
	if result.Error != nil {
...
// FindTradableToday returns tradable new coins for today
func (r *GORMNewCoinRepository) FindTradableToday(ctx context.Context) ([]*models.NewCoin, error) {
	...
	var coins []*models.NewCoin
	result := r.db.WithContext(ctx).
		Where("status = ? AND became_tradable_at BETWEEN ? AND ? AND is_deleted = ?", 
			"1", startOfDay, endOfDay, false).
		Find(&coins)
	if result.Error != nil {
...
// Count returns the count of new coins
func (r *GORMNewCoinRepository) Count(ctx context.Context) (int64, error) {
	var count int64
	result := r.db.WithContext(ctx).Model(&models.NewCoin{}).Where("is_deleted = ?", false).Count(&count)
	if result.Error != nil {
