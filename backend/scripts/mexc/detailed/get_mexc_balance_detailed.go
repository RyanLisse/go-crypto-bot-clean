...
func main() {
	...
	logger := log.With().Str("component", "mexc-balance-detailed-script").Logger()
	...
	if apiKey == "" || apiSecret == "" {
		logger.Fatal().Msg("MEXC_API_KEY and MEXC_API_SECRET environment variables must be set")
	}
	...
	nonZeroBalances := len(wallet.Balances)
	...
	logger.Info().
		...
		Int("nonZeroBalances", nonZeroBalances).
	...
	saveWalletToFile(wallet, "mexc_balance_detailed.json")
	logger.Info().Msg("Wallet data saved to mexc_balance_detailed.json")

	// Save the wallet data as raw JSON for reference
	rawData, _ := json.MarshalIndent(wallet, "", "  ")
	os.WriteFile("mexc_account_raw.json", rawData, 0644)
	logger.Info().Msg("Raw account data saved to mexc_account_raw.json")
}
