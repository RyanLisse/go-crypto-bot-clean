...
func TestGetMarketData(t *testing.T) {
	...
	assert.Equal(t, 42000.0, marketData.Ticker.LastPrice) // Use Price from market.Ticker
	assert.Equal(t, 100.0, marketData.Ticker.PriceChange)
	assert.Equal(t, 2.5, marketData.Ticker.PriceChangePercent) // Use PercentChange from market.Ticker
	assert.Equal(t, 42200.0, marketData.Ticker.HighPrice)   // Use High24h from market.Ticker
	assert.Equal(t, 41700.0, marketData.Ticker.LowPrice)    // Use Low24h from market.Ticker
	...
	assert.NotZero(t, marketData.Ticker.Timestamp)
}
...
func TestGetKlines(t *testing.T) {
	...
	// Test GetKlines
	klines, err := client.GetKlines(context.Background(), "BTCUSDT", "1h", 2)
	...
	assert.Equal(t, model.KlineInterval("1h"), klines[0].Interval)
	...
	assert.Equal(t, model.KlineInterval("1h"), klines[1].Interval)
