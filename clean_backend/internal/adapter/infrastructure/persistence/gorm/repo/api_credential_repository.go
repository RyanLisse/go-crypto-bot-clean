...
type APICredentialRepository struct {
	db         *gorm.DB
	encryption port.EncryptionService
	logger     *zerolog.Logger
}

// NewAPICredentialRepository creates a new APICredentialRepository
func NewAPICredentialRepository(db *gorm.DB, encryption port.EncryptionService, logger *zerolog.Logger) *APICredentialRepository {
...
func (r *APICredentialRepository) ListAll(ctx context.Context) ([]*model.APICredential, error) {
	...
	for _, entity := range entities {
		...
		if !lastVerified.IsZero() {
			credential.LastVerified = &lastVerified
		}
...
func (r *APICredentialRepository) Save(ctx context.Context, credential *model.APICredential) error {
	...
	if credential.LastUsed != nil {
		entity.LastUsed = *credential.LastUsed
	}
	if credential.LastVerified != nil {
		entity.LastVerified = *credential.LastVerified
	}
	if credential.ExpiresAt != nil {
		entity.ExpiresAt = *credential.ExpiresAt
	}
	if credential.RotationDue != nil {
		entity.RotationDue = *credential.RotationDue
	}
