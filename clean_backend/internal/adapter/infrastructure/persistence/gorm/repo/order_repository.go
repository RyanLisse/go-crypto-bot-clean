...
func (r *OrderRepository) toDomain(entity *entity.OrderEntity) *model.Order {
	return &model.Order{
		ID:            entity.ID,
...
func (r *OrderRepository) toEntity(order *model.Order) *entity.OrderEntity {
...
func (r *OrderRepository) Delete(ctx context.Context, id string) error {
	result := r.db.WithContext(ctx).Delete(&entity.OrderEntity{}, "id = ?", id)
	if result.Error != nil {
		r.logger.Error().Err(result.Error).
			Str("orderId", id).
...
func (r *OrderRepository) Count(ctx context.Context, filters map[string]interface{}) (int64, error) {
	var count int64
	query := r.db.WithContext(ctx).Model(&entity.OrderEntity{})
