...
// Error scenarios for TursoDB initialization
func TestNewTursoDBInvalidURL(t *testing.T) {
    logger := zerolog.New(os.Stdout).With().Timestamp().Logger()
    // Initialization should fail with malformed URL
    _, err := NewTursoDB("invalid-url", "invalid-token", 0, &logger)
    require.Error(t, err)
}

func TestNewTursoDBInvalidAuthToken(t *testing.T) {
    logger := zerolog.New(os.Stdout).With().Timestamp().Logger()
    // Use a valid file path URL but invalid token to simulate auth failure
    _, err := NewTursoDB("file://nonexistent/path", "invalid-token", 0, &logger)
    require.Error(t, err)
}
