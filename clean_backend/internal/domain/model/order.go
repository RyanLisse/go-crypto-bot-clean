...
// CancelOrderResult represents the result of an order cancellation attempt.
type CancelOrderResult struct {
	Success   bool   `json:"success"`
	OrderID   string `json:"orderId,omitempty"`   // Original order ID
	ClientOid string `json:"clientOid,omitempty"` // Original client order ID
	Message   string `json:"message,omitempty"`   // Optional message (e.g., reason for failure)
}

// AccountInfo represents basic account details and permissions.
// Add fields as needed based on API responses.
type AccountInfo struct {
	UserID      string    `json:"userId"` // Internal or exchange user ID
	CanTrade    bool      `json:"canTrade"`
	CanWithdraw bool      `json:"canWithdraw"`
	CanDeposit  bool      `json:"canDeposit"`
	Balances    []Balance `json:"balances"` // Slice of asset balances
	LastUpdated time.Time `json:"lastUpdated"`
}

// Balance represents the balance for a specific asset.
// type Balance struct {
...
// OrderResult represents the result of a successful order placement.
