...
type MexcSniperService struct {
	...
	listingDetector  port.ListingDetector
...
func NewMexcSniperService(
	...
	service := &MexcSniperService{
		...
		eventRepo:        eventRepo,
		eventBus:         eventBus,
		listingDetector:  listingDetector,
		logger:           logger,
		config:           config,
		status:           StatusStopped,
		rateLimiter:      rate.NewLimiter(rate.Limit(config.RateLimitPerSec), 20),
		orderSemaphore:   make(chan struct{}, config.MaxConcurrentOrders),
		lastCacheClean:   time.Now(),
		saveOrderQueue:   make(chan *model.Order, 100),
		autoSnipeEnabled: false,
		autoSnipeConfig:  config,
	}
...
func (s *MexcSniperService) waitForOrderFill(ctx context.Context, symbol, orderID string) (*model.Order, error) {
	...
	for {
		select {
		case <-ticker.C:
			// Use GetOrderStatusWithRetry from the injected tradeExecutor
...
func (s *MexcSniperService) SetupAutoSnipe(enabled bool, config *port.SniperConfig) error {
	...
	if enabled && s.listingDetector != nil {
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				coin, err := s.symbolRepo.GetBySymbol(ctx, "BTCUSDT")
				coins := []*model.Symbol{coin}
				if err != nil {
...
// DetectAndProcessNewListings checks for new listings and processes them
func (s *MexcSniperService) DetectAndProcessNewListings(ctx context.Context) error {
	// Use the specific ListingDetector interface
	coins, err := s.listingDetector.GetNewListings(ctx)
	if err != nil {
		return fmt.Errorf("failed to get new listings: %w", err)
	}

	for _, coin := range coins {
		existing, err := s.newCoinRepo.GetBySymbol(ctx, coin.Symbol)
		if err != nil {
			s.logger.Error().Err(err).Str("symbol", coin.Symbol).Msg("Failed to check existing coin")
		...
		if existing == nil {
			...
			if err := s.newCoinRepo.Save(ctx, coin); err != nil {
				s.logger.Error().Err(err).Str("symbol", coin.Symbol).Msg("Failed to save new coin")
				continue // Skip this coin on error
			}
			...
			if err := s.eventRepo.SaveEvent(ctx, event); err != nil {
				// Log error but continue processing, event saving failure shouldn't stop detection flow
				s.logger.Error().Err(err).Str("symbol", coin.Symbol).Msg("Failed to save new_coin_detected event")
			}
			s.eventBus.Publish(event)
			s.logger.Info().Str("symbol", coin.Symbol).Msg("New coin detected and saved")

		} else {
			// Existing coin found, check if status needs update
			if existing.Status != coin.Status {
				...
				if err := s.newCoinRepo.Update(ctx, existing); err != nil {
					s.logger.Error().Err(err).Str("symbol", coin.Symbol).Msg("Failed to update coin status")
					continue // Skip event emission if update failed
				}
				...
				if err := s.eventRepo.SaveEvent(ctx, event); err != nil {
					s.logger.Error().Err(err).Str("symbol", coin.Symbol).Msg("Failed to save status_changed event")
				}
				s.eventBus.Publish(event)
				s.logger.Info().Str("symbol", coin.Symbol).Str("oldStatus", string(oldStatus)).Str("newStatus", string(coin.Status)).Msg("Coin status updated")
			}
		}
	}
