...
const generateInsights = async (portfolioData: any, tradeHistory: any): Promise<Insight[]> => {
  try {
...
export function AIInsights({ className }: AIInsightsProps) {
  ...
  // Generate insights based on portfolio and trade history
  const { data: insights, isLoading, error, refetch } = useQuery({
    queryKey: ['ai-insights', portfolioData, tradeHistory],
    queryFn: () => generateInsights(portfolioData, tradeHistory),
    staleTime: 5 * 60 * 1000, // 5 minutes
    enabled: !!portfolioData && !!tradeHistory, // Only run when portfolio and trade history are available
  });
