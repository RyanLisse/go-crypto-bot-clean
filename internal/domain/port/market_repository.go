...
// TickerRepository handles storage and retrieval of market ticker data
type TickerRepository interface {
	// Create stores a new Ticker
	Create(ctx context.Context, ticker *market.Ticker) error

	// GetBySymbol returns the latest Ticker for a symbol
	GetBySymbol(ctx context.Context, symbol string) (*market.Ticker, error)

	// GetByExchange returns all current Tickers from a specific exchange
	GetByExchange(ctx context.Context, exchange string) ([]*market.Ticker, error)

	// GetHistorical returns historical ticker data for a symbol within a time range
	GetHistorical(ctx context.Context, symbol string, startTime, endTime int64) ([]*market.Ticker, error)

	// UpdateOrCreate updates a ticker if it exists or creates a new one
	UpdateOrCreate(ctx context.Context, ticker *market.Ticker) error
}
